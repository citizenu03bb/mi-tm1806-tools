...
def run_rgbkb(*args, timeout=5):
    """Run rgbkb CLI and return True on success."""
    cmd = ["sudo", str(SCRIPT_DIR / ".." / "rgbkb" / "rgbkb")] + list(args)
    try:
...
class EffectsComposer(QMainWindow):
    ...
    def _paint_frame(self, frame):
        """Send one frame to the keyboard hardware."""
        for i, zone in enumerate(ZONE_NAMES):
            run_rgbkb("zone", zone, "0x" + frame["zones"][i])
        effect = self.effect_combo.currentText()
        speed = self.speed_combo.currentText()
        if effect != "static":
            run_rgbkb(effect, "0x" + frame["zones"][0], "--speed", speed)
        else:
            run_rgbkb("solid", "0x" + frame["zones"][0])
    ...
    def _export_script(self):
        ...
        lines = ["#!/bin/bash",
                 ...
                 "set -eu",
                 f'RGBKB="$(cd "$(dirname "$0")" && pwd)/../rgbkb/rgbkb"',
        ...
        if effect != "static" and len(self.frames) == 1 and \
           len(set(self.frames[0]["zones"])) == 1:
            ...
            lines.append(f'sudo "$RGBKB" {effect} "0x{c}" --speed {speed}')
        else:
            lines.append("while true; do")
            for f in self.frames:
                dur = f["duration"]
                # Check if all zones same colour
                if len(set(f["zones"])) == 1:
                    lines.append(f'    "$RGBKB" solid "0x{f["zones"][0]}"'
                                 f' >/dev/null 2>&1 || true')
                else:
                    for i, zone in enumerate(ZONE_NAMES):
                        lines.append(f'    "$RGBKB" zone {zone} "0x{f["zones"][i]}"'
                                     f' >/dev/null 2>&1 || true')
                if effect != "static":
                    c0 = f["zones"][0]
                    lines.append(f'    "$RGBKB" {effect} "0x{c0}" --speed {speed}'
                                 f' >/dev/null 2>&1 || true')
